...
NEW in this version:
- Language selector dropdown with 23 supported languages
- Automatic language detection based on text (optional)
- Language-specific sample texts
- Compatible with multilingual TTS server
- All previous features maintained (streaming, chunking, voice profiles, etc.)
"""
...
def call_tts_standard(
    ...
    payload = {
        ...
        "chunk_by_sentences": bool(chunk_by_sentences),  # mostly ignored by server in non-stream
    }
...
class MultilingualVoiceTestApp(tk.Tk):
    def __init__(self):
        ...
        # NEW: streaming toggle (default = non-streaming)
        self.streaming_enabled = tk.BooleanVar(value=False)
        # Chunking only makes sense when streaming
    ...
    def _build_widgets(self):
        ...
        self.language_combo.set("🇬🇧 English")  # Default to English
        ...
        # Sample text selector (dynamic based on language)
        ...
        # Chunking toggle (only relevant when streaming)
        ...
        # Initialize chunking checkbox state
    ...
    def _on_language_changed(self, event=None):
        """Update sample buttons when language changes."""
        self._update_sample_buttons()

    def _update_sample_buttons(self):
        """Rebuild sample text buttons based on current language."""
    ...
    # ----------------- Streaming toggle / chunking -----------------

    def _on_streaming_toggle(self):
        """Enable/disable chunking checkbox depending on streaming state."""
    ...
    def _on_speak(self):
        """Generate and play speech with language support."""
        ...
        # Decide mode: streaming vs non-streaming (default = non-streaming)
        use_streaming = bool(self.streaming_enabled.get())

        # Chunking flag only matters in streaming mode; otherwise it's ignored.
        chunk_by_sentences = bool(self.chunking_enabled.get()) if use_streaming else False

        # Start playback thread (works for both streaming and non-streaming:
        # non-streaming will just push one full WAV into the queue)
    ...
    def _worker_speak(
        ...
        """Background worker for TTS generation (streaming or non-streaming)."""
        try:
            ...
            if use_streaming:
                ...
                self.after(
                    ...
                    lambda: self.status_var.set(
                        f"🎵 Generating {voice_name} ({lang_display}, {mode_label})..."
                    ),
                )
                ...
                def on_chunk(chunk_num, chunk_bytes):
                    ...
                    self.after(
                        0,
                        lambda: self.status_var.set(
                            f"🎵 Playing chunk {chunk_num} ({lang_display}, {mode_label})..."
                        ),
                    )
                ...
                # Signal end of stream to the audio player
                ...
                wav_bytes = call_tts_standard(
                    ...
                    language,
                    profile,
                    chunk_by_sentences=False,  # explicit, though server ignores for non-stream
                )

                if cancel_event.is_set():
                    return

                # Play the whole file as a single chunk
    ...
    def _on_speak_complete(
        ...
        self.status_var.set(
            f"✓ Completed ({chunk_count} chunk(s), {voice_name}, {lang_display}, {mode_label})"
        )
